...
<div class="profil">
        <form action="" method="POST" enctype="multipart/form-data">
            <table border="1">
                ...
                <td>Kategori</td>
                <td>
                <!-- <input type="text" name="kategori_id" value="<?= $data_buku["kategori_id"] ?>"> -->
                    <select name="kategori_id" id="">
    
                    <?php
                        foreach($data_kategori as $k){
                            ?>
                            <option value="<?= $k["id"] ?>" <?= ($data_buku['kategori_id'] == $k['id']) ? "selected" : "" ; ?>>
                                <?= $k['id'] ?> | <?= $k['nama'] ?>
                            </option>                            
                            <?php
                    }
                ...
                <td>Nama Penerbit</td>
                <td>
                   <!-- <input type="text" name="penerbit_id" value="<?= $data_buku["penerbit_id"] ?>"> -->
                  <select name="penerbit_id"  id="">
                <?php
                        foreach($data_penerbit as $p){
                            ?>
                            <option value="<?= $p["id"] ?>" <?= ($data_buku['penerbit_id'] == $p['id']) ? "selected" : "" ;?>>
                                <?= $p['id'] ?> | <?= $p['nama'] ?>
                        </option>
                        <?php
                    }
